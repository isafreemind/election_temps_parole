...
export const RatioChart = ({ candidates }: RatioChartProps) => {
  const data = candidates.map((c, idx) => {
    const ratio = (c.tempsTotalSeconds / c.voix);
    return {
      nom: c.nom,
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div style={{
          ...
          <p style={{ color: '#94a3b8', margin: '3px 0' }}>Ratio: {data.ratio.toFixed(4)} sec/voix</p>
          <p style={{ color: '#94a3b8', margin: '3px 0' }}>Temps: {data.heures}h</p>
          <p style={{ color: '#94a3b8', margin: '3px 0' }}>Voix: {data.voixK}k</p>
        </div>
      );
    }
    return null;
    ...
    <ResponsiveContainer width="100%" height={450}>
      ...
      >
        ...
        <XAxis 
          ...
          stroke="#94a3b8"
          style={{ fontSize: '12px' }}
          label={{ value: 'Secondes de parole par voix', position: 'insideBottom', offset: -5, fill: '#94a3b8' }}
        />
        <YAxis 
          type="category"
          dataKey="nom"
